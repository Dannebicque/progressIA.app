...
final class Version20260831133758 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if (!$schema->hasTable('answer_choice')) {
        ...
        if (!$schema->hasTable('course_institution')) {
        ...
        if (!$schema->hasTable('course_semester')) {
        ...
        if (!$schema->hasTable('course_formation')) {
        ...
        if (!$schema->hasTable('evaluation_attempt')) {
        ...
        if (!$schema->hasTable('landing_config')) {
        ...
        if (!$schema->hasTable('page_completion')) {
        ...
        if (!$schema->hasTable('user_institution')) {
    ...
    public function down(Schema $schema): void
    {
        ...
        if ($schema->hasTable('ai_usage_log')) { $this->addSql('DROP TABLE ai_usage_log'); }
        if ($schema->hasTable('answer_choice')) { $this->addSql('DROP TABLE answer_choice'); }
        if ($schema->hasTable('badge')) { $this->addSql('DROP TABLE badge'); }
        if ($schema->hasTable('chapter')) { $this->addSql('DROP TABLE chapter'); }
        if ($schema->hasTable('contact_request')) { $this->addSql('DROP TABLE contact_request'); }
        if ($schema->hasTable('content_page')) { $this->addSql('DROP TABLE content_page'); }
        if ($schema->hasTable('course')) { $this->addSql('DROP TABLE course'); }
        if ($schema->hasTable('course_institution')) { $this->addSql('DROP TABLE course_institution'); }
        if ($schema->hasTable('course_semester')) { $this->addSql('DROP TABLE course_semester'); }
        if ($schema->hasTable('course_formation')) { $this->addSql('DROP TABLE course_formation'); }
        if ($schema->hasTable('course_teacher')) { $this->addSql('DROP TABLE course_teacher'); }
        if ($schema->hasTable('email_template')) { $this->addSql('DROP TABLE email_template'); }
        if ($schema->hasTable('evaluation')) { $this->addSql('DROP TABLE evaluation'); }
        if ($schema->hasTable('evaluation_attempt')) { $this->addSql('DROP TABLE evaluation_attempt'); }
        if ($schema->hasTable('formation')) { $this->addSql('DROP TABLE formation'); }
        if ($schema->hasTable('institution')) { $this->addSql('DROP TABLE institution'); }
        if ($schema->hasTable('landing_config')) { $this->addSql('DROP TABLE landing_config'); }
        if ($schema->hasTable('page_completion')) { $this->addSql('DROP TABLE page_completion'); }
        if ($schema->hasTable('question')) { $this->addSql('DROP TABLE question'); }
        if ($schema->hasTable('semester')) { $this->addSql('DROP TABLE semester'); }
        if ($schema->hasTable('sent_email')) { $this->addSql('DROP TABLE sent_email'); }
        if ($schema->hasTable('session')) { $this->addSql('DROP TABLE session'); }
        if ($schema->hasTable('user')) { $this->addSql('DROP TABLE `user`'); }
        if ($schema->hasTable('user_institution')) { $this->addSql('DROP TABLE user_institution'); }
    }
}
